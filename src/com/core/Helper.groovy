...
class Helper implements Serializable {
    ...
    def Static String getQATestsBranch(env, qa_tests_branch) {
        node('master') {
            // helper function to find corresponding qatests branch to be used for testing dev branch
            def result = ""
            withCredentials([usernamePassword(credentialsId: 'cfb2df52-09d4-4f27-ad17-71a58c4995d9', passwordVariable: 'password', usernameVariable: 'username')]) {
                script {
                    if (env.GIT_BRANCH.toLowerCase() == "dev" || env.GIT_BRANCH == "master") {
                        ...
                        if (qa_tests_branch == "") {
                            ...
                            if (env.GIT_BRANCH =~ /^\w+(-|_)\d+/) {
                                echo "dev branch has matched jira ticket name convention"
                                ...
                                echo "dev branch has not matched jira ticket name convention"
                                branch_prefix = env.GIT_BRANCH
                                (filter1, filter2) = [branch_prefix, branch_prefix]
                            }
                            def branch_qa_hash = sh(returnStdout: true, script:"git ls-remote https://${username}:${password}@github.com/nexmoinc/qatests.git | grep \"$filter1\\|$filter2\" || echo 'switch to qatests master'").toString().trim()
                            if (branch_qa_hash == "switch to qatests master") {
                                echo "No corresponding qatests branch found -> using master"
                                qatests_target_branch = "master"
                            } else {
                                echo "found qatests branch ${branch_qa_hash}"
